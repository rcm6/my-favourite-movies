...
// global variable to store the movie search history to local storage
...
function getMovieInfo(movie) {
  // created a variable here to initialise the countId
  ...
  $.ajax({
    ...
    if (response.imdbID) {
      ...
      if (movieHistory.filter((e) => e.name === movie).length === 0) {
        ...
        // if there is more than six object one will be removed and there will always be only six objects stored in movieHistory array
...
function renderMovieCards() {
  ...
  console.log("movieHistory array: ", movieHistory);
